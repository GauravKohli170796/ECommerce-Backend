...
export class WishListService {
  ...
  async getUserWishListItems(email: string): Promise<unknown> {
    const response = await this.wishListModel
      .find({ email: email })
      .populate("productId", { name: 1, description: 1, images: { $slice: 1 } });
    return response;
  }
  ...
  async addItemToUserWishList(email: string, wishListItem: IWishListItem): Promise<unknown> {
    ...
    return await (
      await this.wishListModel.create(addWishListItem)
    ).populate("productId", { name: 1, description: 1, images: { $slice: 1 } });
  }
  ...
  async deleteUserWishList(email: string): Promise<unknown> {
    const deleteResponse = await this.wishListModel.deleteMany({ email: email });
    return deleteResponse;
  }
}
